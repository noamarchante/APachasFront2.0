...
export class DetailEventComponent implements OnInit {
    ...
    addEventGoogleCalendar(event: MEvent) {
        let date: string = "${this.event.eventStart.replace(new RegExp(' ', 'g'), 'T').replace(new RegExp(':', 'g'), '').replace(new RegExp('-', 'g'), '').substring(0,9) + (parseInt(this.event.eventStart.replace(new RegExp(' ', 'g'), 'T').replace(new RegExp(':', 'g'), '').replace(new RegExp('-', 'g'), '').substring(9))+20000).toString()}" + "/" + "${this.event.eventEnd.replace(new RegExp(' ', 'g'), 'T').replace(new RegExp(':', 'g'), '').replace(new RegExp('-', 'g'), '').substring(0,9) + (parseInt(this.event.eventEnd.replace(new RegExp(' ', 'g'), 'T').replace(new RegExp(':', 'g'), '').replace(new RegExp('-', 'g'), '').substring(9))+20000).toString()}";
        ...
        this.googleCalendarUrl = +"?text=" + title + "&dates=" + date + "&details=" + description + "&location=" + location + "&sf=true&output=xml";
    }
}