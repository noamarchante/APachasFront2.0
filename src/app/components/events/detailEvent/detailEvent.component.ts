...
export class DetailEventComponent implements OnInit {
    ...
    addEventGoogleCalendar(event: MEvent){
        ...
        if(this.event){
            this.googleCalendarUrl = "https://calendar.google.com/calendar/u/0/r/eventedit?text=" + 
            this.event.eventName.replace(new RegExp(' ', 'g'), '+') + 
            "&dates=" + 
            this.event.eventStart.replace(new RegExp(' ', 'g'), 'T').replace(new RegExp(':', 'g'), '').replace(new RegExp('-', 'g'), '').substring(0,9) + 
            (parseInt(this.event.eventStart.replace(new RegExp(' ', 'g'), 'T').replace(new RegExp(':', 'g'), '').replace(new RegExp('-', 'g'), '').substring(9))+20000).toString() + 
            "/" + this.event.eventEnd.replace(new RegExp(' ', 'g'), 'T').replace(new RegExp(':', 'g'), '').replace(new RegExp('-', 'g'), '').substring(0,9) + 
            (parseInt(this.event.eventEnd.replace(new RegExp(' ', 'g'), 'T').replace(new RegExp(':', 'g'), '').replace(new RegExp('-', 'g'), '').substring(9))+20000).toString() + 
            "&details=" + 
            this.event.eventDescription.replace(new RegExp(' ', 'g'), '+') + 
            "&location=" + 
            this.event.eventLocation.replace(new RegExp(' ', 'g'), '+') + 
            "&sf=true&output=xml";
        }
    }
}